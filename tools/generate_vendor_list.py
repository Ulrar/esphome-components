...
def generate_python_dict(vendors):
    ...
    lines = ["# Known UPS vendor IDs for validation", "# Auto-generated from ups_vendors.h", "KNOWN_VENDOR_IDS = {"]
...
def update_udev_rules_in_setup_sh(setup_path, udev_rules):
    ...
    # Replace the section
    new_content = content[:start_pos] + new_udev_section + content[end_pos + len(udev_end):]
...
def main():
    ...
    if init_path.exists():
        ...
        if re.search(pattern, content, re.DOTALL):
            new_content = re.sub(pattern, python_code.split('\n', 2)[2], content, flags=re.DOTALL)
    ...
    if update_udev_rules_in_setup_sh(setup_path, udev_rules):
        ...
        print(f"   Added {len([r for r in udev_rules if 'UPS' not in r])} standard + {len(vendors)} UPS vendor rules")
